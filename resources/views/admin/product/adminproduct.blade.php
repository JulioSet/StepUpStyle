    ...
    <div class="card container-fluid">
        ...
        <div class="card-body w-100 ">
            <div class="d-flex container-fluid p-2">
                ...
                <a href="/admin/product/add" class="btn btn-primary ml-auto mb-1">Add</a>
            </div>
            <table id="myTable" class="table table-bordered table-clickable">
                ...
                <tbody>
                    @foreach ($listproduk as $item)
                        <tr data-href="{{ route('viewVarianProduct', $item->sepatu_id) }}" style="cursor: pointer">
                            <td>{{$loop->iteration}}</td>
                            <td>{{$item->sepatu_name}}</td>
                            <td>{{$item->supplier->supplier_name}}</td>
                            <td>{{$item->kategori->kategori_nama}}</td>
                            <td>{{$item->subkategori->subkategori_nama}}</td>
                            <td>
                                <div class="d-flex">
                                    <a href="{{route('viewEditProduct',$item->sepatu_id)}}" class="btn btn-primary mr-3">
